### Reference : MSNet: multispectral semantic segmentation network for remote sensing images
### Code : https://github.com/taochx/MSNet/
...
class rgb_net(nn.Module):
    def __init__(self, num_classes, filters=16):
        super().__init__()
        self.rgb = resnet50(weights=ResNet50_Weights.IMAGENET1K_V2)

        # decoder
        self.dec5 = DecoderBlock(2048, filters * 8,upscale=False)

        self.dec4 = DecoderBlock(2048 + filters * 8, filters * 32)
        self.dec3 = DecoderBlock(1024 + filters * 8, filters * 16)
        self.dec2 = DecoderBlock(512 + filters * 4, filters * 8)
        self.dec1 = DecoderBlock(256 + filters * 2, filters * 4)
    ...
    def forward(self, rgb):
        ...
        dec5 = self.dec5(nn.functional.max_pool2d(rgb4, kernel_size=3, stride=1,dilation=1,padding=1))
...
class nnn_net(nn.Module):
    def __init__(self, num_classes, filters=16):
        super().__init__()
        self.nnn = resnet50(weights=ResNet50_Weights.IMAGENET1K_V2)
        #patch from the authors to make non-visible bands 1-channel
    ...
    def forward(self, nnn):
        ...
        dec5 = self.dec5(nn.functional.max_pool2d(nnn4, kernel_size=3, stride=1,dilation=1,padding=1))
