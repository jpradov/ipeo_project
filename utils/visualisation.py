...
def visualise_image(image, label):
    ...
    normalized = normalized_image(image.squeeze().numpy())

    fig, axes = plt.subplots(
        nrows=1, ncols=3, sharex=True, sharey=True, figsize=(18, 8)
    )
    axes[0].imshow(normalized[[0, 1, 2], :, :].transpose(1, 2, 0))
    axes[0].set_title("True Color Image")
    axes[0].axis("off")

    axes[1].imshow(normalized[[1, 2, 3], :, :].transpose(1, 2, 0))
    axes[1].set_title("False Color Image (NINFR, RED, GREEN)")
    axes[1].axis("off")

    axes[2].imshow(label.squeeze().numpy())
...
def visualise_image_3_channels(image, label):
    ...
    axes[1].imshow(label.squeeze().numpy())
